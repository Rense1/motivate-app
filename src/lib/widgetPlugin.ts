...
interface WidgetPlugin {
  ...
  /** 現在表示中のタスクタイトル一覧を Android 側に渡す */
  setCurrentTasks(options: { tasks: string[] }): Promise<void>
}
...
/** タスク一覧を SharedPreferences に保存してウィジェットを更新 */
export async function syncWidgetTasks(taskTitles: string[]): Promise<void> {
  if (!Capacitor.isNativePlatform()) return
  try {
    await Widget.setCurrentTasks({ tasks: taskTitles })
  } catch {
    // プラグイン未登録環境では無視
  }
}
